...
def _flashattn_body(batch, heads, kv_head_num, seqlen_kv, dim, pe_dim, block_N, block_H, num_split):
    ...
    def main_no_split(
        ...
        with T.Kernel(heads // min(block_H, kv_group_num), batch, threads=128) as (hid, bid):
            ...
            for k in T.serial(loop_range):
                T.copy(KV[bid, (2 * k) * block_N : (2 * k + 1) * block_N, cur_kv_head, :h_dim], KV_shared_0_l)
                T.copy(KV[bid, (2 * k) * block_N : (2 * k + 1) * block_N, cur_kv_head, h_dim:], KV_shared_0_r)
                T.copy(K_pe[bid, (2 * k) * block_N : (2 * k + 1) * block_N, cur_kv_head, :], K_pe_shared_0)

                T.copy(KV[bid, (2 * k + 1) * block_N : (2 * k + 2) * block_N, cur_kv_head, :h_dim], KV_shared_1_l)
                T.copy(KV[bid, (2 * k + 1) * block_N : (2 * k + 2) * block_N, cur_kv_head, h_dim:], KV_shared_1_r)
                ...
                T.gemm(acc_s_1_cast, KV_shared_1_r, acc_o_r)
...
def flashattn_auto(batch, heads, kv_head_num, seqlen_kv, dim, pe_dim, block_N, block_H, num_split):
    return _flashattn_body(batch, heads, kv_head_num, seqlen_kv, dim, pe_dim, block_N, block_H, num_split)


# --- Without auto_schedule ---
@tilelang.jit(
    out_idx=[6],
)
def flashattn_manual(batch, heads, kv_head_num, seqlen_kv, dim, pe_dim, block_N, block_H, num_split):
    return _flashattn_body(batch, heads, kv_head_num, seqlen_kv, dim, pe_dim, block_N, block_H, num_split)


def _flashattn_warp_specialize_body(batch, heads, kv_head_num, seqlen_kv, dim, pe_dim, block_N, block_H, num_split):
    scale = (1.0 / (dim + pe_dim)) ** 0.5 * 1.44269504  # log2(e)
    dtype = T.float16
    accum_dtype = T.float32
    kv_group_num = heads // kv_head_num
...
# --- Warp-specialized (manual, no auto_schedule) ---
@tilelang.jit(
    out_idx=[6],
)
def flashattn_warp_specialize(batch, heads, kv_head_num, seqlen_kv, dim, pe_dim, block_N, block_H, num_split):
    return _flashattn_warp_specialize_body(batch, heads, kv_head_num, seqlen_kv, dim, pe_dim, block_N, block_H, num_split)
...
def main(batch=1, heads=64, kv_heads=1, kv_ctx=1024, dim=512, pe_dim=64):
    ...
    results = []  # list of (label, latency_ms, tflops, correct)

    for kernel_fn, label in configs:
        print("=" * 60)
        ...
        print(f"  [{status}] latency: {latency:.4f} ms  |  {tflops:.2f} TFlops")

    # ------- Print ranking table -------
    results.sort(key=lambda x: x[2], reverse=True)  # sort by TFlops descending

    print()
    print("=" * 60)
    print("             FlashMLA Decode Benchmark Results")
    print(f"  batch={batch}, heads={heads}, kv_heads={kv_heads}")
    print(f"  kv_ctx={kv_ctx}, dim={dim}, pe_dim={pe_dim}")
    print("=" * 60)
    print(f"{'Rank':<6}{'Configuration':<22}{'Latency(ms)':>12}{'TFlops':>10}{'Correct':>10}")
    print("-" * 60)
    for rank, (label, lat, tfl, cor) in enumerate(results, 1):
        status = "PASS" if cor else "FAIL"
        print(f"{rank:<6}{label:<22}{lat:>12.4f}{tfl:>10.2f}{status:>10}")
    print("-" * 60)
